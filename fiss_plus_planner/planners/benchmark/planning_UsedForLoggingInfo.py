...
def frenet_optimal_planning(scenario: Scenario, planning_problem: PlanningProblem, vehicle_params: DictConfig, method: str, num_samples: tuple, 
    ...
    traj_log_path = os.path.join(output_dir, f"{scenario_name}_{method}_traj_log.csv")
    traj_log_lines = ["time_step,x1,y1,d,v,t\n"]
    ...
    for i in range(final_time_step):
        ...
        end_time = time.time()
        if best_traj_ego is not None and len(best_traj_ego.x) > 1:
            sampling_param = best_traj_ego.sampling_param
            traj_log_lines.append(
                f"{i},{best_traj_ego.x[1]:.6f},{best_traj_ego.y[1]:.6f},"
                f"{sampling_param.d:.6f},{sampling_param.s_d:.6f},{sampling_param.t:.6f}\n"
            )
    ...
    #TODO: please write also the tag of the scenario in the traj_log_lines
